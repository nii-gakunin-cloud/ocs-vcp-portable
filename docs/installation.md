...
|registry-vcpmirror|registry|5000(TCP)|各VCノードから利用することを想定、VCPネットワーク内で利用|
|registry-local|registry|5001(TCP)|各VCノードから利用することを想定、VCPネットワーク内で利用|
...
起動したコンテナ群のうち、Web UI を持つものについて説明する。  

#### Grafana

VCPで起動したマシンのメモリ使用量等のメトリクスは `prometheus` が収集しており、`Grafana` はそのデータを可視化するためのダッシュボードを提供する。  

`http://{VCコントローラのアドレス}:8080/grafana/` をWebブラウザで開くと利用できる。  
...
詳細は [Grafana](./services/grafana.md) を参照。

#### Jupyter

VCPの利用者は、`vcpsdk`（VCコントローラ用のPythonクライアントライブラリ）を用いてVCコントローラへリクエストを送ることで、マシンの起動等を行う。本リポジトリでは、その実行環境として `Jupyter Notebook` を用意している。  

`http://{VCコントローラのアドレス}:8080/jupyter/` をWebブラウザで開くと、VCP SDKが利用可能なJupyterLab環境にアクセスできる。
...
VCPでは、マシンの起動・停止等の管理を「Unitgroup」という単位で行い、その状態をConsul KVSに保存している。実際のマシンの状態とVCP上の管理情報に不整合が生じた場合は、Consul KVSを直接更新して復旧させる必要がある。この操作はVC管理者の役割である。  

`http://{VCコントローラのアドレス}:8080/consul/` をWebブラウザで開くと、Consulの管理画面（サービスカタログ・KVSの状態確認等）にアクセスできる。
...
詳細は [Consul](./services/consul.md) を参照。
